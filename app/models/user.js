...
var findOrCreate = function(data, callback){
	findOne({'sid': data.id}, function(err, user){
		...
		if(user){
			...
			if(data.provider == "facebook" && userData.pic){
				userData.pic = "http://graph.facebook.com/" + data.id + "/picture?type=large";
			}
